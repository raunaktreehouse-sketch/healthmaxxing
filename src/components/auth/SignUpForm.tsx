...
export function SignUpForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await signIn("credentials", { email, password, callbackUrl: "/forum", redirect: true })
    } catch {
  ...
  async function handleOAuth(provider: string) {
    await signIn(provider, { callbackUrl: "/forum" })
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="username" value={username} onChange={e => setUsername(e.target.value.replace(/[^a-z0-9_]/gi, '').toLowerCase())} placeholder="johndoe" required />
          </div>
        </div>
        <div className="space-y-1.5">
